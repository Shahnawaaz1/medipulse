...
export default function PublicLandingPage() {
  ...
  const handleBookAppointment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const patientId = patData.data?._id || patData.existingId;
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm dark:bg-slate-900/95 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/dashboard"
              className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-brand-600 to-teal-500 px-4 py-2.5 text-xs font-extrabold text-white shadow-md shadow-brand-500/20 hover:brightness-110 transition-all"
            >
              <Layers className="h-4 w-4" />
              <span>Hospital ERP Suite</span>
            </Link>
          </div>
        </div>
      </header>
